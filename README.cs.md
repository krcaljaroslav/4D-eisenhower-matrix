...
| **Graf závislostí** | Ukazuje cíle nad jejich blokátory v pravoúhlé mřížce a úkoly bez vazeb v odděleném pásmu. Úrovně, jejichž tasky mají předchozí tasky, zůstávají v jedné řadě a pokračují doprava (posuvník nebo tažení plátna); tasky bez předchozích jsou pod sebou pod taskem, který blokují, a jejich spojnice vedou z boku. Větve lze sbalit, graf zoomovat a na desktopu karty ručně rozmístit; první ruční posun přidá `🆔`, souřadnice zůstávají v `data.json`. Mobil nabízí čtení, navigaci, menu a zakládání bez dragu. |
...
**1.0.38-1.0.39** - Navázané tasky přímo z kontextového menu: nový poslední blok **Add previous task** / **Add follow-up task** otevře formulář u karty a nový task naváže zvoleným směrem, v matici, Kanbanu i grafu (v grafu nahrazuje *Add blocker below* / *Add dependent above*); pole závislostí v editaci se k tomu přejmenovala na **Previous task** / **Follow-up task**. Graf závislostí rozmisťuje po větvích místo roztahování jedné úrovně přes celou obrazovku: úrovně, jejichž tasky mají předchozí tasky, zůstávají v jedné řadě a pokračují doprava (dostaneš se k nim posuvníkem nebo tažením plátna), tasky bez předchozích jsou ve sloupci přímo pod taskem, který blokují, a jejich spojnice vedou z boku karty do mezery a teprve pak nahoru, takže už nevypadají, že vedou ke kartě nad nimi. Řady mají větší rozestupy, tlačítko sbalení větve je poloviční a stojí vedle příchozí spojnice a ručně umístěné karty zůstávají, kde jsi je položil. (1.0.38 krátce zalamovala široké úrovně do víc řad; 1.0.39 to nahrazuje.)

**1.0.36-1.0.37** - Přidán graf závislostí vzniklý ve vývojovém cyklu 1.0.33, se zakládáním tasků a našeptáváním tagů, ovládáním jednotlivých větví, sdíleným odpočtem dokončení, centrovaným resetem a zoomem, posuvníky a tažením plátna, konzistentním tlačítkem Back a širšími compact kartami bez nadbytečných textových popisků vazeb. Verze 1.0.37 přinesla úklid nálezů z kontroly pluginu od Obsidianu, ovládání se nemění: obě potvrzovací hlášky (reset pozic v grafu, dokončení blokovaného tasku) používají místo prohlížečového `confirm()` Obsidian modal, který se v popout okně neotvírá nad špatným oknem a na mobilu ho nejde potlačit; graf ze stejného důvodu plánuje animační snímky přes `window`; zmizela obě pravidla s `!important`; `minAppVersion` se posouvá na **1.8.7**, verzi, ve které vzniklo použité `Notice` API.
